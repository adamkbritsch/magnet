...
run settings   "$(dirname "$0")/settings/main.swift"   "$SRC/Settings.swift" "$SRC/SiteStyle.swift" "$SRC/Mirrors.swift" "$SRC/Categories.swift" "$SRC/Bookmarks.swift"
run wikipedia  "$(dirname "$0")/wikipedia/main.swift"  "$SRC/Settings.swift" "$SRC/SiteStyle.swift" "$SRC/Mirrors.swift"
run fmhy       "$(dirname "$0")/fmhy/main.swift"       "$SRC/Settings.swift" "$SRC/SiteStyle.swift" "$SRC/Mirrors.swift"
run categories "$(dirname "$0")/categories/main.swift" "$SRC/Settings.swift" "$SRC/SiteStyle.swift" "$SRC/Mirrors.swift" "$SRC/Categories.swift" "$SRC/Bookmarks.swift"
...
run banners    "$(dirname "$0")/banners/main.swift"    "$SRC/Banners.swift"
run downloads  "$(dirname "$0")/downloads/main.swift"  "$SRC/Settings.swift" "$SRC/SiteStyle.swift" "$SRC/Mirrors.swift" "$SRC/Categories.swift" "$SRC/Bookmarks.swift" "$SRC/Downloads.swift"
if [ "${1:-}" = "--live" ]; then
  run live "$(dirname "$0")/live/main.swift" "$SRC/Settings.swift" "$SRC/SiteStyle.swift" "$SRC/Mirrors.swift"
